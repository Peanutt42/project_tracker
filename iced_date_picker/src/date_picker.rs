...
use crate::overlay::{self, DatePickerOverlay, DatePickerOverlayButtons};
...
pub use crate::{
	date::Date,
	style::{Status, Style, StyleFn},
};

pub fn date_picker<'a, Message, Theme, F>(
	show_picker: bool,
	date: impl Into<crate::date::Date>,
...
where
	...
	F: 'static + Fn(crate::date::Date) -> Message,
{
	crate::DatePicker::new(show_picker, date, underlay, on_submit, on_close)
}
		...
		+ iced::widget::container::Catalog,
{
	...
	where
		<Theme as crate::style::Catalog>::Class<'a>: From<StyleFn<'a, Theme, Style>>,
	{
	...
	#[must_use]
	pub fn class(mut self, class: impl Into<<Theme as crate::style::Catalog>::Class<'a>>) -> Self {
...
pub struct State<Message> {
	...
	pub(crate) overlay_state: overlay::State<Message>,
}
...
impl<Message> State<Message> {
	...
	pub fn now() -> Self {
		Self {
			overlay_state: overlay::State::<Message>::default(),
		}
	}

	/// Creates a new [`State`] with the given date.
	#[must_use]
	pub fn new(date: Date, on_submit: Rc<dyn Fn(Date) -> Message>, on_close: Message) -> Self {
		Self {
			overlay_state: overlay::State::new(date.into(), on_submit.clone(), on_close),
		}
	}
...
{
	fn tag(&self) -> Tag {
		Tag::of::<State<Message>>()
	}

	fn state(&self) -> widget::tree::State {
		widget::tree::State::new(State::<Message>::new(
			self.date,
	...
	fn overlay<'b>(
		...
		let picker_state: &mut State<Message> = state.state.downcast_mut();
