...
impl CreateTaskModal {
	...
	pub fn view<'a>(&'a self, database: &'a Option<Database>, preferences: &'a Option<Preferences>)
		-> Element<'a, Message>
	{
		...
		let due_date_view = due_date_button(
			...
			|date| CreateTaskModalMessage::ChangeDueDate(SerializableDate::from_iced_date(date)).into(),
			CreateTaskModalMessage::ClearDueDate.into()
		);

		card(
			if let Some(project) = database.as_ref().and_then(|db| db.get_project(&self.project_id)) {
				...
				vertical_scrollable(
					column![
						...
						text_editor(&self.task_description)
							.on_action(|action| CreateTaskModalMessage::TaskDescriptionAction(action).into())
							.style(description_text_editor_style)
							.key_binding(|key_press| text_editor_keybindings(key_press, CreateTaskModalMessage::UnindentDescription.into())),
