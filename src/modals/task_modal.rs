...
impl TaskModal {
	...
	pub fn update<'a>(&'a mut self, message: TaskModalMessage, database: &'a Option<Database>) -> TaskModalAction {
		match message {
			...
			TaskModalMessage::EditDescriptionAction(action) => if let Some(new_description) = &mut self.new_description {
			...
			TaskModalMessage::UnindentDescription => if let Some(new_description) = &mut self.new_description {
	...
	pub fn view<'a>(&'a self, app: &'a ProjectTrackerApp) -> Element<'a, Message> {
		card(
			...
			if let Some(project) = app.database.as_ref().and_then(|db| db.get_project(&self.project_id)) {
				if let Some(task) = project.get_task(&self.task_id) {
					...
					let description_text: Element<'a, Message> = if let Some(new_description) = &self.new_description {
						...
						task_description(app.task_description_markdown_items.get(&self.task_id), app)
					};
