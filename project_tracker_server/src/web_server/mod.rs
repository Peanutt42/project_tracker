...
pub async fn run_web_server(password: String, modified_receiver: Receiver<ModifiedEvent>, shared_data: Arc<RwLock<SharedServerData>>) {
	...
	let admin_infos_route = path("admin_infos")
		...
		.map(move |body: serde_json::Value, shared_data: Arc<RwLock<SharedServerData>>| {
	...
	let modified_ws_route = path("modified")
		...
		.and(shared_data_clone)
		.and(password_clone)
		.and_then(move |ws: Ws, client_password: String, client_addr: Option<SocketAddr>, modified_receiver: Arc<RwLock<Receiver<ModifiedEvent>>>, shared_data: Arc<RwLock<SharedServerData>>, password: String| {
			async move {
				if client_password == password {
					Ok(ws.on_upgrade(move |socket|
						on_upgrade_modified_ws(socket, client_addr, modified_receiver.read().unwrap().resubscribe(), shared_data)
					))
				}
				else {
	...
	let login_html_route = path("login")
		.and(end())
		.map(|| html(LOGIN_INDEX_HTML));

	let login_index_route = login_path
		.and(end())
		.map(|| html(LOGIN_INDEX_HTML));
	...
	let admin_html_route = path("admin")
		.and(end())
		.map(|| html(ADMIN_INDEX_HTML));

	let admin_index_route = admin_path
		.and(end())
		.map(|| html(ADMIN_INDEX_HTML));
...
fn get_admin_infos(body: serde_json::Value, password: String, shared_data: Arc<RwLock<SharedServerData>>) -> Response {
	if body.get("password") == Some(&serde_json::Value::String(password)) {
		let (cpu_usage, connected_clients) = {
			...
			(shared_data.cpu_usage_avg, shared_data.connected_clients.clone())
		};
...
async fn on_upgrade_modified_ws(ws: WebSocket, client_addr: Option<SocketAddr>, modified_receiver: Receiver<ModifiedEvent>, shared_data: Arc<RwLock<SharedServerData>>) {
