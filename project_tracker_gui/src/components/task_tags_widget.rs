...
pub fn task_tags_buttons<'a>(available_tags: &'a OrderedHashMap<TaskTagId, TaskTag>, tags: &'a HashSet<TaskTagId>, on_press: impl Fn(TaskTagId) -> UiMessage) -> Element<'a, UiMessage> {
	Row::with_children(
		available_tags
			.iter()
			.map(|(tag_id, tag)| {
